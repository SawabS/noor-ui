...
`Tooltip` - `content`, wraps a single trigger child; mount one `TooltipProvider` near the app root.
`Toast` - `ToastProvider` + `useToast()` + `Toast` visual variants matching Alert's semantics.
`EmptyState`, `ErrorState` - `icon`, heading, description, optional `action`/`onRetry`.

## Overlays

`Dialog`, `Drawer` - `trigger`, `title` (required for a11y), `description`, `open`/`onOpenChange`; `Drawer` adds `side` (`start`/`end`/`top`/`bottom`).
`Popover` - `trigger`, `content`, `side`, `align`.
`DropdownMenu`, `ContextMenu` - composable: `DropdownMenu`/`DropdownMenuItem`/`DropdownMenuSeparator`/`DropdownMenuLabel`/`DropdownMenuCheckboxItem`/`DropdownMenuRadioItem` (and the `ContextMenu` equivalents).
`CommandPalette` - `items` (grouped), `open`/`onOpenChange`, optional `enableShortcut` for Cmd/Ctrl+K.
...
`TopNavigation` - `start`/`center`/`end` slot props.
`UserMenu` - dropdown-based account menu.

## Data display

`Card` (+ `CardHeader`/`CardTitle`/`CardDescription`/`CardContent`/`CardFooter`) - `padding`, `interactive`.
...
`ChatLayout` - top-level shell composing `sidebar`, message area, and `composer` slots, responsive (sidebar collapses on mobile).
